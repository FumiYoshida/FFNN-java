...
public class FastMeowSandBox extends AbstractSamplePlayer {
	...
	public Action doMyTurn() {
		...
		if (grace < 3 && board.getTotalNumberOfOjama() >= ojamathreshold) {
			...
			for (int i=0;i<myactionnum;i++) {
				...
				firstscores[i] = Math.max(firstscores[i], tempfirstscore);
				for (int j=0;j<mysecondfields.length;j++) {
					...
					for (int k=0;k<mythirdfields.length;k++) {
						...
						int temp = (int)(tempfirstscore * 1.1 + tempsecondscore * 1.05 + tempthirdscore + mythirdfields[k].firepossibility);
						if (temp > mymaxsumscores[i]) {
							mymaxsumscores[i] = temp;
						}
					}
				}
			}
					...
					if (firstscores[selectindex] < enemymaxscore) {
						...
						for (int i=0;i<myactionnum;i++) {
							if (firstscores[i] == 0 && mymaxsumscores[i] > nextturnmaxscore) {
								nextturnmaxscore =mymaxsumscores[i];
								tempindex = i;
							}
						}
	...
	public static void main(String args[]) {
		AbstractPlayer player1 = new FastMeowSandBox();
