...
public class Meow extends AbstractSamplePlayer {
	...
	public Action doMyTurn() {
		...
		if (grace < 4 && board.getTotalNumberOfOjama() >= ojamathreshold) {
			...
			if (grace == 1) {
				...
				int ojamanum3 = ojamalist.get(2);
				for (int i=0;i<myactionnum;i++) {
					bnf.Calc(firstfields[i],  false, false);
					int ojamadan = (ojamanum - firstfields[i].score / 70 + 5) / 6;
					int firstscorecarry = 0;
					if (firstfields[i].score > ojamanum * 70) {
						ojamadan = 0;
						firstscorecarry = firstfields[i].score - ojamanum * 70;
					}
					...
					for (int j=0;j<secondfields.length;j++) {
						...
						int ojamadan2 = (ojamanum2 - (firstscorecarry + secondfields[j].score) / 70 + 5) / 6;
						int secondscorecarry = 0;
						if (firstscorecarry + secondfields[j].score > ojamadan2 * 70) {
							secondscorecarry = firstscorecarry + secondfields[j].score - ojamadan2 * 70;
							ojamadan2 = 0;
						}
						...
						for (int k=0;k<thirdfields.length;k++) {
							bnf.CalcPlacetoFire(thirdfields[k], ojamanum3, secondscorecarry);
							int[] fourthscores = bnf.Tsumos(thirdfields[k]);	
							int temp = secondfields[j].score + thirdfields[k].score + 2000 - bnf.OjamaDiscount(thirdfields[k]);		
							if (!needtofire) {
								temp += thirdfields[k].firepossibility;
							}
					...
					maxsumscores[i] = firstfields[i].score + sumscore / 15;
				}
			}
			else if (grace == 2) {
				boolean needtofire = true;
				...
				for (int i=0;i<myactionnum;i++) {
					...
					for (int j=0;j<secondfields.length;j++) {
						bnf.Calc(secondfields[j], false, false);
						int ojamadan2 = (ojamanum2 - (firstfields[i].score + secondfields[j].score) / 70 + 5) / 6;
						int secondscorecarry = 0;
						if (firstfields[i].score + secondfields[j].score > ojamadan2 * 70) {
							secondscorecarry = firstfields[i].score + secondfields[j].score - ojamadan2 * 70;
							ojamadan2 = 0;
						}
						bnf.FallDownOjama(secondfields[j], ojamadan2);
						bnf.ThinkNextActions(secondfields[j]);
				...
				for (int i=0;i<myactionnum;i++) {
					...
					for (int j=0;j<mysecondfields.length;j++) {
						...
						for (int k=0;k<mythirdfields.length;k++) {
							bnf.CalcPlacetoFire(mythirdfields[k], ojamanum, myfirstfields[i].score + mysecondfields[j].score);
							int[] fourthscores = bnf.Tsumos(mythirdfields[k]);	
							int temp = mysecondfields[j].score + mythirdfields[k].score + 2000 - bnf.OjamaDiscount(mythirdfields[k]);		
							if (!needtofire) {
								temp += mythirdfields[k].firepossibility;
							}
							bnf.CompareScores(maxscoresofeachtsumo, fourthscores, temp);
						}
					}
			...
			for (int i=0;i<enemyactionnum;i++) {
				...
				if (CountEnemyBlank() > 40 && false) {
					Arrays.sort(maxscoresofeachtsumo);
					enemymaxsumscores[i] = tempfirstscore + maxscoresofeachtsumo[10];
				}
				else {
					int sumscore = 0;
					for (int j=0;j<15;j++) {
	...
	public int CountEnemyBlank() {
		Field f = getEnemyBoard().getField();
		int width = f.getWidth();
		int height = f.getHeight() - 2;
		int output = 0;
		for (int i=0;i<width;i++) {
