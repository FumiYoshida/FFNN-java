...
public class Meow4 extends AbstractSamplePlayer {
	...
	public Action doMyTurn() {
		...
		EasyBitNextField bnf = new EasyBitNextField();
			...
			for (int i=0;i<enemyactionnum;i++) {
				...
				for (int j=0;j<enemysecondfields.length;j++) {
					...
					for (int k=0;k<enemythirdfields.length;k++) {
						bnf.Calc(enemythirdfields[k], false, false);
			...
			for (int i=0;i<myactionnum;i++) {
				...
				for (int j=0;j<mysecondfields.length;j++) {
					...
					for (int k=0;k<mythirdfields.length;k++) {
						bnf.Calc(mythirdfields[k], false, true);
						int tempthirdscore = mythirdfields[k].score;
						thirdscores[k] = mythirdfields[k].score;
						thirdevaluations[k] = tempfirstscore * 1.1 + tempsecondscore * 1.05 + tempthirdscore + mythirdfields[k].firepossibility;
					}
					
					int[] goodmoves = IndexofSorted(thirdevaluations);
					for (int k0=0;k0<Math.min(mythirdfields.length, 6);k0++) {
						int k = goodmoves[Math.max(0, mythirdfields.length - 1 - k0)];
						
						// ネクネクの次は最悪のつもが来ると仮定して、α/β法を用いる
						// まずは最初のつも(青、緑)に対して、自分の最善手を考える。
						FieldInfo fourthfield = bnf.Tsumos(mythirdfields[k], 0);
						FieldInfo[] fourthfields = fourthfield.AvailableFields(0, 0);
						double fourthevaluation = 0;
						for (int m=0;m<fourthfields.length;m++) {
							bnf.Calc(fourthfields[m], false, true);
							double tempfourtheva = tempfirstscore * 1.1 + tempsecondscore * 1.05 + thirdscores[k] + fourthfields[m].score * 0.95 + fourthfields[m].firepossibility * 0.95;
							fourthevaluation = Math.max(fourthevaluation, tempfourtheva);
						}
						for (int l=1;l<15;l++) {
							// 各つもについて見る
							fourthfield = bnf.Tsumos(mythirdfields[k], l);
							fourthfields = fourthfield.AvailableFields(0, 0);
							boolean isthistsumoworst = true;
							double evaluationofthistsumo = 0; // 現在のつもに対する評価
							for (int m=0;m<fourthfields.length;m++) {
								bnf.Calc(fourthfields[m], false, true);
								double tempfourtheva = tempfirstscore * 1.1 + tempsecondscore * 1.05 + thirdscores[k] + fourthfields[m].score * 0.95 + fourthfields[m].firepossibility * 0.95;
								evaluationofthistsumo = Math.max(evaluationofthistsumo, tempfourtheva);
								if (tempfourtheva > fourthevaluation) {
									// もしこのつもに対して、今まで考えた中で最悪のつもに対する手よりも良い手があったら、
									// (最悪のつもが来ると仮定しているので)このつもは来ないことになる。
									isthistsumoworst = false;
									break;
								}
							}
							if (isthistsumoworst) {
								// 現在のつもが今までに見たつもの中で最悪のものであった場合
								fourthevaluation = evaluationofthistsumo;
							}
						}
						mymaxsumscores[i] = Math.max(mymaxsumscores[i], fourthevaluation);
					}
				}
			}
	...
	public static void main(String args[]) {
		AbstractPlayer player1 = new Meow4();
