...
public class FastMeow extends AbstractSamplePlayer {
			...
			for (int i=0;i<enemyactionnum;i++) {
				...
				for (int j=0;j<enemysecondfields.length;j++) {
					...
					for (int k=0;k<enemythirdfields.length;k++) {
						bnf.Calc(enemythirdfields[k], enemythirdojamafield, false, true, false, 0, 0);
						int tempthirdscore = bnf.score;
						int temp = tempfirstscore + tempsecondscore + tempthirdscore;
						enemymaxfirepossibility = Math.max(enemymaxfirepossibility, temp + bnf.fastfirepossibility);
			...
			for (int i=0;i<myactionnum;i++) {
				...
				for (int j=0;j<mysecondfields.length;j++) {
					...
					for (int k=0;k<mythirdfields.length;k++) {
						bnf.Calc(mythirdfields[k], mythirdojamafield, false, false, false, 0, 0);
						int tempthirdscore = bnf.score;
	...
	public static void main(String args[]) {
		AbstractPlayer player1 = new FastMeow();
